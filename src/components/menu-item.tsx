...
export default function MenuItem(props: MenuItemProps) {
    ...
    <main>
        <h2 className="text-xl font-bold">{item.name} - ${item.price}</h2>
            ...
            <div className="mb-4 ">
              {item.isVegan && (
                <button className="mr-2 bg-green-500 text-white px-2 py-1 rounded">v</button>
              )}
              {item.isVegetarian && item.isVegan == false && (
                <button className="mr-2 bg-blue-500 text-white px-2 py-1 rounded">V</button>
              )}
              {item.isGlutenFree && (
                <button className="bg-yellow-500 text-white px-2 py-1 rounded">gf</button>
              )}
              {item.isDairyFree && (
                <button className="bg-orange-500 text-white px-2 py-1 rounded">df</button>
              )}
